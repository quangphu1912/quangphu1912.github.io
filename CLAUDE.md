...
**Covers (`.hero-home` + `.projects-hero`) - shared layout + brand-blue surface, distinct composition.** Both full-bleed covers share the `.hero-home, .projects-hero { … }` **layout** shell (bleed-behind-header `margin-top`, `min-height:100vh/svh`, white text) and the **same brand-blue navy surface** - the `min-height` is what gives the work-rows slide its headroom (the first row must start below the fold), so **don't shrink it**. They're differentiated by **composition, not colour**, so Home→Projects never reads as a reload: **Home = identity** - centered stack (`.hero-home__inner` is `align-items:center`), with the mono-blue glow *slowly drifting* via the `heroAurora` keyframe on `.hero-home::before` (reduced-motion-gated, transform/opacity only so it's VT-snapshot-safe); **Projects = a "selected work" index** - `.projects-hero__inner` is **left-aligned** (`align-items:flex-start`), eyebrow ("Selected work") → title → lead → a single-hue `.projects-hero__cats` discipline legend (brand-blue dots, **not** the per-category triad - those hues stay on the cards). The only surface tweak is `.projects-hero::before` mirroring Home's glow to the top-right so it isn't a pixel-copy. (An earlier tri-color teal/violet/amber `.projects-hero` wash was reverted - keep the cover brand-blue.) Per-cover entrances stay transform-only (no opacity) so the VT snapshot is never blank.
