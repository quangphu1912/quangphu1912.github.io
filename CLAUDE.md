...
- **Body:** opens straight into tags → `{{ content }}` → prev/next. Credits are **not** repeated here (they live on the cover). No Timeline row: the cover eyebrow already shows the date. The cover has **no** Back-to-Projects link - the transparent header's Projects nav is the way back (and it's themed to the project). An image-less project (no `page.image`) falls back to a body `.detail-back-row` back-link + plain `<h1>` instead of the cover.
