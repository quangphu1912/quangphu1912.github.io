...
- `header.html` / `footer.html` - transparent scroll-away header (a `.back-to-top` chevron - michellegore's bottom-right muted control - lives inline in `default.html` after the footer: pure CSS, `href="#top"` + smooth scroll, scroll-driven fade-in past the first viewport); footer is a minimal one-line `.footer-bar` warmed with a single message: **left** a `.footer-message` ("Thanks for stopping by, let's chat. 👋"), **middle** the `.footer-icons` (LinkedIn · GitHub · Email, icon-only via `social-url.html`; the mail icon carries no address - `nav.js` reassembles `data-user`/`data-domain` into a `mailto:` on click), **right** the `.footer-copy` © {year} {name}. `justify-content: space-between` lays the three out on one row. (An earlier warm 3-column headline version was reverted to this single line.)
...
**Covers (`.hero-home` + `.projects-hero`) - shared layout + brand-blue surface, distinct composition.** Both full-bleed covers share the `.hero-home, .projects-hero { … }` **layout** shell (bleed-behind-header `margin-top`, `min-height:100vh/svh`, white text) and the **same brand-blue navy surface** - the `min-height` is what gives the work-rows slide its headroom (the first row must start below the fold), so **don't shrink it**. Both covers also **share the same surface** (navy gradient + blue glow + dot-grid) - there is **no** tri-color `.projects-hero` wash (an experiment that was reverted). **Home = identity**, distinguished by the mono-blue glow that *slowly drifts* via the `heroAurora` keyframe on `.hero-home::before` (reduced-motion-gated, transform/opacity only so it's VT-snapshot-safe; Projects keeps the glow static). **Projects = the work listing**, opening with a centered title + lead, then the `.projects-hero__cats` Data/ML/BI legend in the per-category dot hues (`--proj-data` teal / `--proj-ml` violet / `--proj-bi` amber). The legend sits **after** the heading (title leads, taxonomy closes the stack) - the one change from the upfront-legend layout that originally shipped. Per-cover entrances stay transform-only (no opacity) so the VT snapshot is never blank.

The **About intro (`.about-intro`)** carries the same identity surface so all three top-of-page moments match: it gets the navy gradient + a static blue glow (`.about-intro::before`) and bleeds up behind the header via the same `margin-top: calc(-1 * var(--header-h))`, but is **content-height, not a 100vh cover** (the about-grid follows immediately, no scroll-cue) and its background **fades into `--color-bg` at the bottom** (`linear-gradient(to bottom, transparent 70%, var(--color-bg))`) so the band melts into the grid rather than hard-cutting. No text-colour overrides are needed - the dark-theme tokens are already light, so name/title/chips read on the navy as-is.
...
**Reach (deliberate).** Category hue touches **only project surfaces**: cover art + eyebrow + wash, listing thumbnail art + hover affordances + the **card category chip** (`.project-card__tag`, michellegore's tinted discipline pill: hue text on the hue at 10%, labels mirroring the projects-hero legend), write-up **section headings** (`.project-content > h2`, her accent-colored case-study subheads), and - on project detail pages only - the **header nav** (hover text + underline + active). **Brand blue stays the global UI accent** everywhere else - buttons, focus rings, the page-transition edge, telemetry, hero, and the global `.eyebrow` (home/section kickers). The nav is the one UI surface that adopts the category hue, scoped to `body[data-category]` (project detail pages); home / about / the projects listing keep white-hover + brand-blue active. The listing card kicker is muted and **date-only** (the discipline moved into the category chip, 2026-07-01 - previously the kicker carried `date · tag` and the chip-free "kicker stays muted" restraint). Do **not** recolor the global `.eyebrow` to a category hue - it would bleed into home/section headers.
