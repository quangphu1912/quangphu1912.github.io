...
- `reveal.js` - `IntersectionObserver` scroll-reveal for `[data-reveal]` elements; gracefully degrades under `prefers-reduced-motion`. In-view elements at load reveal via two paths split by **arrival type**, so the entrance is smooth either way. **Cross-document View Transition (in-site nav / back-forward):** a **synchronous inline `<script>` at end-of-`<body>` in `_layouts/default.html`** adds `is-visible` to in-view rows during parse, **before the incoming VT snapshot is captured**, so they're solid in the snapshot and rise *with* the page (no slide, no flash). This MUST be inline+classic, not a deferred module: Safari captures the snapshot before `reveal.js` (a module) runs, so a module-based reveal lands too late = the Safari flash that nav-type detection in the module couldn't fix. **Cold load / refresh / typed-URL (no VT):** `reveal.js` reveals on a post-paint double `requestAnimationFrame` so the work-rows slide-in entrance actually plays (revealing pre-paint makes them pop in fully formed - the "abrupt" landing 13d7006 fixed). On a VT nav both run: the inline reveal wins pre-snapshot, the module's rAF re-adds `is-visible` as a no-op. Off-screen rows reveal on scroll via the observer.
